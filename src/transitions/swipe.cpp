...
obs_source_t *swipe_transition_source_create(obs_data_t *settings)
{
	const char *direction =
		obs_data_get_string(settings, S_SWIPE_DIRECTION);
	...
	obs_source_t *source =
		obs_source_create_private(SWIPE_TRANSITION_ID, NULL, data);
	obs_data_release(data);

	return source;
}

void swipe_transition_setup_properties(transition *transition,
				       obs_properties_t *props, bool visible)
{
	obs_property_t *p = obs_properties_add_list(props, S_SWIPE_DIRECTION,
						    SWIPE_LOCALE("Direction"),
						    OBS_COMBO_TYPE_LIST,
