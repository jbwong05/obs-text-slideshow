...
void slide_transition_setup_properties(transition *transition, obs_properties_t *props, bool visible) {
    obs_property_t *p = obs_properties_add_list(props, S_SLIDE_DIRECTION, SLIDE_LOCALE("Direction"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
...
transition *create_slide_transition() {
    transition *slide_transition = (transition *)bzalloc(sizeof(transition));
